...
export async function generateCoverLetter(input: unknown) {
  const data = CoverLetterInput.parse(input);
...
export async function generateInterviewQuestions(input: unknown) {
  const data = InterviewPrepInput.parse(input);
  const gateway = createLovableAiGatewayProvider(requireLovableApiKey(), undefined, {
    structuredOutputs: true,
  });
...
export async function evaluateInterviewAnswer(input: unknown) {
  const data = AnswerEvaluationInput.parse(input);
  const gateway = createLovableAiGatewayProvider(requireLovableApiKey(), undefined, {
    structuredOutputs: true,
  });
