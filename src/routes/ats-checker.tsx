...
export const Route = createFileRoute("/ats-checker")({
  head: () => ({
    meta: [
      { title: "Free AI ATS Resume Checker & LaTeX Resume Builder | FixMyResume" },
      {
        name: "description",
        content:
          "Check your resume ATS score free, fix missing keywords with AI, and build a clean one-page LaTeX resume you can download as PDF. No signup required.",
      },
      {
        name: "keywords",
        content:
          "free ats resume checker, check ats score online, resume keyword match scanner, ats optimizer, rate my resume, check my cv score, ats compliance checker, scan resume for keywords, fixmyresume",
      },
      {
        property: "og:title",
        content: "Free AI ATS Resume Checker & LaTeX Resume Builder | FixMyResume",
      },
      {
        property: "og:description",
        content:
          "Instant ATS scoring, keyword gap analysis, AI resume rewrites and an AI-powered LaTeX resume editor.",
      },
      { property: "og:type", content: "website" },
      { property: "og:url", content: `${SITE_URL}/ats-checker` },
      { property: "og:site_name", content: SITE_NAME },
      { name: "twitter:card", content: "summary_large_image" },
      { name: "twitter:title", content: "Free AI ATS Resume Checker & LaTeX Resume Builder" },
      {
        name: "twitter:description",
        content: "Score your resume like an ATS, fix the gaps with AI and export a clean PDF.",
      },
          ...
          "@graph": [
            {
              ...
              description:
                "Free AI ATS resume checker, resume optimiser and LaTeX resume builder with A4 PDF export.",
...
function Index() {
  ...
  const latexFn = useServerFn(generateLatex);

  const analysis = useMutation({
    mutationFn: () => analyzeFn({ data: { resume, jobDescription } }),
    onError: (error: Error) => toast.error(error.message || "Analysis failed. Please try again."),
  });

  const improve = useMutation({
    mutationFn: () => improveFn({ data: { resume, jobDescription } }),
    onSuccess: (result) => {
  ...
  const latexGen = useMutation({
    mutationFn: (instruction: string) =>
      latexFn({
        data: { instruction, currentCode: latex, resume: improved || resume },
      }),
    onSuccess: (result) => {
