...
const TITLE = "Free AI Resume Builder — Templates & A4 PDF Download | FixMyResume";
const DESCRIPTION =
  "Build an ATS-friendly resume in minutes: describe your background, let AI write it, pick from 11 templates (with or without a photo) and download an A4 PDF free.";
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: DESCRIPTION },
      {
        name: "keywords",
        content:
          "free ai resume builder, online cv maker, ats friendly resume template, resume creator online, free cv builder, ai cv writer, download resume pdf, simple resume builder, modern resume templates, a4 resume editor, fixmyresume",
      },
      ...
      { name: "twitter:title", content: "Free AI Resume Builder with ATS Templates" },
    ...
    scripts: [
      {
        ...
        children: JSON.stringify({
          ...
          "@type": "WebApplication",
          name: `${SITE_NAME} AI Resume Builder`,
          url: `${SITE_URL}/`,
...
function BuilderPage() {
  ...
  const parsePythonFn = useServerFn(parseResumePdfWithPython);
  const buildFn = useServerFn(buildResume);
  const polishFn = useServerFn(polishResume);
  ...
  async function handleDirectUpload(file: File | undefined) {
    ...
    try {
      ...
      const result = await parsePythonFn({ data: { pdfBase64: base64 } });
      if (result.error) {
  ...
  const build = useMutation({
    mutationFn: () => buildFn({ data: { brief, targetRole } }),
    onSuccess: (result) => {
      setData({ ...result, photo: data.photo });
      registerBuildResume();
  ...
  const polish = useMutation({
    mutationFn: () => polishFn({ data: { data, targetRole } }),
    onSuccess: (result) => {
      setData({ ...result, photo: data.photo });
      registerPolishResume();
