...
export const Route = createFileRoute("/interview-prep")({
  head: () => ({
    meta: [
      { title: `AI Mock Interview Prep & Evaluator | ${SITE_NAME}` },
      {
        name: "description",
        content: "Generate tailored interview questions based on your resume and target job, draft answers, and get real-time AI feedback with score metrics.",
      },
    ],
    links: [{ rel: "canonical", href: `${SITE_URL}/interview-prep` }],
  }),
...
function InterviewPrepPage() {
  ...
  const evaluateAnswerFn = useServerFn(evaluateInterviewAnswer);
  
  const isPremium = subscriptionTier === "pro" || subscriptionTier === "elite";

  const generate = useMutation({
    mutationFn: () => generateQuestionsFn({ data: { resumeData, jobDescription } }),
    onSuccess: (result) => {
  ...
  const evaluate = useMutation({
    mutationFn: (q: Question) =>
      evaluateAnswerFn({
        data: { question: q.question, userAnswer, tips: q.tips },
      }),
    onSuccess: (result, q) => {
